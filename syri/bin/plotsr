...
if __name__ == '__main__':
    ...
    df['alen'] = df[2] - df[1]
    df['blen'] = df[7] - df[6]
    df = df.loc[(df['alen'] >= args.s) | (df['blen'] >= args.s)]
    ...
    ax.set_yticklabels([i[0] for i in chrid])
        ...
        for row in df.loc[(df[0] == chrid[i][0]) & (df[10] == 'SYN')].itertuples(index=False):
            if not adSynLab:
                p = Polygon(np.array([[row[1], len(chrid)-0.02-i],
                         ...
                         [row[4], len(chrid)-0.02-i-0.7,]]), closed=True, color=COLORS[0], linewidth=0, label='Syntenic')
                adSynLab=True
            else:
                p = Polygon(np.array([[row[1], len(chrid)-0.02-i],
                         [row[2], len(chrid)-0.02-i],
                         [row[5], len(chrid)-0.02-i-0.7],
                         [row[4], len(chrid)-0.02-i-0.7,]]), closed=True, color=COLORS[0], linewidth=0)
            ax.add_patch(p)
    ...
    fig.savefig('syri.pdf', bbox_inches='tight')
