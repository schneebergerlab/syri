...
def getBlocks(orderedBlocks, isinv, annoCoords, threshold, tUC, tUP):
    if len(orderedBlocks) == 0:
        return([])

    outOrderedBlocks = makeBlocksTree_ctx(orderedBlocks.aStart.values, orderedBlocks.aEnd.values, orderedBlocks.bStart.values, orderedBlocks.bEnd.values, orderedBlocks.bDir.values, orderedBlocks.aChr.values, orderedBlocks.bChr.values, threshold)
