...
#         long                            endacoord, endbcoord
#         float                           score_i, minvalue
...
#             break
#
#     print(int(st.size()), str(datetime.now()))
#     path.clear()
#     lp = st.size()
...
#     profit_list = np.array([endb[i] for i in range(<Py_ssize_t> endb.size())], dtype=np.float32)
#
#     parents = np.array([-1]*lp, dtype = 'int')
#
#     print('starting')
...
#             # print(i, deref(staenda_it2).first, deref(staenda_it2).second.size(), count, count1)
#             for j in range(<Py_ssize_t> deref(staenda_it2).second.size()):
#                 current = deref(staenda_it2).second[j]
#                 canmap_rit = canmap.rbegin()
#                 while canmap_rit != canmap.rend():
#                     if deref(canmap_rit).first > end_list[current]:
...
#                                 changed = False
#                                 if deref(canmap_rit2).first > endb_list[current]:
...
#             inc(staenda_it2)
#
#     print('finding goodinv', str(datetime.now()))
#     maxid = -1
#     maxscore = -1
#     for i in range(lp):
#         if totscore[i] > maxscore:
...
cpdef getProfitable_BKP(invblocks, long[:] aStart, long[:] aEnd, long[:] bStart, long[:] bEnd, float[:] iDen, cpp_map[int, cpp_vec[long]] neighbourSyn, float[:] synBlockScore, long[:] aStartSyn, long[:] aEndSyn, long tUC, float tUP, long threshold, brk = -1):
    cdef:
        long                            i, j, k, l, current, count
        long                            n_topo, n_edges, n_syn
        long                            leftSyn, rightSyn, leftEnd, rightEnd, overlapLength
